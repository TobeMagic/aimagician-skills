...
from pptx_studio.curation import (
...
from pptx_studio.composition import CompositionError, compile_composition, verify_composition_replay_lock
...
from pptx_studio.narrative import validate_narrative_plan
from pptx_studio.runtime import runtime_health
from pptx_studio.style_planning import plan_style_cluster
from pptx_studio.component_profiles import load_component_profiles, query_component_profiles
from pptx_studio.component_curation import compile_component_profile
from pptx_studio.physical_adapter import (
    assemble_from_plans,
    certified_timeline_cardinality_adaptation_keys,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("command", choices=("runtime-check", "plan", "apply", "verify", "recover", "promote-components", "verify-component-promotion", "render", "compile", "refresh-render-index", "rebuild-catalog", "query", "query-components", "query-batch", "inspect-deck", "plan-style-cluster", "probe-cover", "probe-page", "compose", "verify-replay", "preflight", "validate-fact-store", "validate-narrative", "bind-outline", "adapt", "assemble", "vision-plan", "vision-prompt", "vision-ingest", "vision-run", "vision-run-range", "curate-components"))
    ...
    parser.add_argument(
        "--certification-evidence", type=Path,
        help="curated certified-core ledger applied deterministically while compiling a catalog",
    )
    parser.add_argument("--catalog", type=Path)
    parser.add_argument("--query-input", type=Path)
    parser.add_argument("--query-output", type=Path)
    parser.add_argument("--style-cluster-input", type=Path, help="role/capacity-only narrative selection request")
    parser.add_argument("--style-cluster-output", type=Path, help="safe compiler-feasible page selection")
    parser.add_argument("--cover-probe-input", type=Path, help="one candidate cover plus actual client title/metadata facts")
    parser.add_argument("--cover-probe-output", type=Path, help="physical cover feasibility result")
    parser.add_argument("--page-probe-input", type=Path, help="one non-structural candidate plus actual client facts")
    parser.add_argument("--page-probe-output", type=Path, help="physical non-structural page feasibility result")
    ...
    parser.add_argument("--component-profile", type=Path, help="private hash-bound component/host-anchor authority")
    parser.add_argument("--component-curation-input", type=Path, help="operator-only source/shape closure declaration")
    parser.add_argument("--component-profile-output", type=Path, help="compiled private component profile output")
    parser.add_argument("--component-promotion-input", type=Path, help="operator-only archive package selection")
    parser.add_argument("--component-promotion-output", type=Path, help="hash-bound private component-promotion manifest")
    parser.add_argument("--replay-output", type=Path, help="explicit PASS or migration-required replay report")
    ...
    parser.add_argument("--brief-normalized", type=Path, help="client-only normalized brief for narrative validation")
    parser.add_argument("--narrative-input", type=Path, help="narrative beats with no requested slide count")
    parser.add_argument("--narrative-output", type=Path, help="validated narrative ledger/report")
    ...
    parser.add_argument(
        "--page-id", action="append",
        help="operator-only exact page ID scope for a hash-bound vision re-observation",
    )
...
def run(argv: Sequence[str] | None = None) -> dict[str, Any]:
    ...
    if args.command == "runtime-check":
        if args.runtime_output is None:
            raise ValueError("RUNTIME_OUTPUT_REQUIRED")
        result = runtime_health(explicit=args.private_root)
        _write_json(args.runtime_output, result)
        return {"status": result["status"], "runtime_output": str(args.runtime_output), "summary": {key: result[key] for key in ("catalog_page_count", "observation_count", "source_package_count")}}
    if args.command == "curate-components":
        if any(value is None for value in (
            args.catalog, args.asset_index, args.private_root,
            args.component_curation_input, args.component_profile_output,
        )):
            raise ValueError("COMPONENT_CURATION_ARGUMENT_REQUIRED")
        result = compile_component_profile(
            catalog=_read_json(args.catalog), asset_index=_read_json(args.asset_index),
            private_root=args.private_root, request=_read_json(args.component_curation_input),
            private_source_root=args.private_source_root,
        )
        _write_json(args.component_profile_output, result)
        return {
            "status": "PASS", "component_profile_output": str(args.component_profile_output),
            "summary": {"component_count": len(result["components"]), "host_anchor_count": len(result["host_anchors"])},
        }
    if args.command == "promote-components":
        if args.component_promotion_input is None or args.component_promotion_output is None:
            raise ValueError("COMPONENT_PROMOTION_ARGUMENT_REQUIRED")
        plan = plan_component_promotion(
            _read_json(args.component_promotion_input), args.source_root,
            archive_root=args.archive_root,
        )
        result = apply_component_promotion(plan, args.source_root, archive_root=args.archive_root)
        _write_json(args.component_promotion_output, result)
        return {
            "status": result["status"],
            "component_promotion_output": str(args.component_promotion_output),
            "summary": verify_component_promotion(result, args.source_root, archive_root=args.archive_root),
        }
    if args.command == "verify-component-promotion":
        if args.component_promotion_output is None:
            raise ValueError("COMPONENT_PROMOTION_ARGUMENT_REQUIRED")
        return verify_component_promotion(
            _read_json(args.component_promotion_output), args.source_root,
            archive_root=args.archive_root,
        )
    ...
    if args.command == "rebuild-catalog":
        if (
            args.render_index is None
            or args.catalog_output is None
            or args.certification_evidence is None
        ):
        ...
        catalog = compile_catalog(
            args.source_root,
            render_index=pages,
            certification_evidence=_read_json(args.certification_evidence),
        )
    ...
    if args.command == "query-components":
        if args.catalog is None or args.observation_index is None or args.component_profile is None or args.query_input is None or args.query_output is None:
            raise ValueError("COMPONENT_QUERY_ARGUMENT_REQUIRED")
        catalog = _read_json(args.catalog)
        profiles = load_component_profiles(args.component_profile, catalog=catalog)
        result = query_component_profiles(
            profiles, catalog=catalog, observations=_complete_observations(args.observation_index),
            request=_read_json(args.query_input),
        )
        _write_json(args.query_output, result)
        return {"status": result["status"], "query_output": str(args.query_output), "summary": {"candidate_count": len(result["candidates"])}}
    ...
    if args.command == "plan-style-cluster":
        if args.catalog is None or args.observation_index is None or args.style_cluster_input is None or args.style_cluster_output is None:
            raise ValueError("STYLE_CLUSTER_ARGUMENT_REQUIRED")
        cardinality_adaptations = (
            certified_timeline_cardinality_adaptation_keys(
                private_source_root=args.private_source_root,
            )
            if args.private_source_root is not None else ()
        )
        result = plan_style_cluster(
            _read_json(args.catalog), observations=_complete_observations(args.observation_index),
            request=_read_json(args.style_cluster_input),
            sequence_cardinality_adaptation_keys=cardinality_adaptations,
        )
        _write_json(args.style_cluster_output, result)
        return {
            "status": result["status"], "style_cluster_output": str(args.style_cluster_output),
            "summary": {"slide_count": len(result.get("recommended_slides", [])), "code": result.get("code")},
        }
    if args.command == "compose":
        ...
        catalog = _read_json(args.catalog)
        component_profiles = (
            load_component_profiles(args.component_profile, catalog=catalog)
            if args.component_profile is not None else None
        )
        result = compile_composition(
            catalog, observations=observations, request=_read_json(args.composition_input),
    ...
    if args.command == "probe-cover":
        if any(value is None for value in (
            args.catalog, args.observation_index, args.cover_probe_input,
            args.cover_probe_output, args.private_source_root, args.assembly_workspace,
        )):
            raise ValueError("COVER_PROBE_ARGUMENT_REQUIRED")
        from pptx_studio.cover_probe import probe_cover_candidate

        result = probe_cover_candidate(
            _read_json(args.catalog), observations=_complete_observations(args.observation_index),
            request=_read_json(args.cover_probe_input),
            private_source_root=args.private_source_root, workspace=args.assembly_workspace,
        )
        _write_json(args.cover_probe_output, result)
        return {
            "status": result["status"], "cover_probe_output": str(args.cover_probe_output),
            "summary": {"candidate_id": result["candidate_id"], "code": result.get("code")},
        }
    if args.command == "probe-page":
        if any(value is None for value in (
            args.catalog, args.observation_index, args.page_probe_input,
            args.page_probe_output, args.private_source_root, args.assembly_workspace,
        )):
            raise ValueError("PAGE_PROBE_ARGUMENT_REQUIRED")
        from pptx_studio.cover_probe import probe_page_candidate

        result = probe_page_candidate(
            _read_json(args.catalog), observations=_complete_observations(args.observation_index),
            request=_read_json(args.page_probe_input),
            private_source_root=args.private_source_root, workspace=args.assembly_workspace,
        )
        _write_json(args.page_probe_output, result)
        return {
            "status": result["status"], "page_probe_output": str(args.page_probe_output),
            "summary": {
                "candidate_id": result["candidate_id"], "role": result.get("role"),
                "code": result.get("code"),
            },
        }
    if args.command == "verify-replay":
        if args.catalog is None or args.observation_index is None or args.composition_plan is None or args.replay_output is None:
            raise ValueError("REPLAY_ARGUMENT_REQUIRED")
        observation_payload = _read_json(args.observation_index)
        entries = observation_payload.get("observations")
        if observation_payload.get("status") != "COMPLETE" or not isinstance(entries, list):
            raise ValueError("OBSERVATION_INDEX_INCOMPLETE")
        ...
        catalog = _read_json(args.catalog)
        plan = _read_json(args.composition_plan)
        component_profiles = (
            load_component_profiles(args.component_profile, catalog=catalog)
            if args.component_profile is not None else None
        )
        try:
            verify_composition_replay_lock(
                plan, catalog=catalog, observations=observations,
                component_profiles=component_profiles,
            )
        except CompositionError as exc:
            result = {
                "schema_version": "pptx-studio-replay-report.v1",
                "status": "MIGRATION_REQUIRED",
                "plan_sha256": __import__("hashlib").sha256(
                    json.dumps(plan, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
                ).hexdigest(),
                "code": str(exc),
                "action": "replan_from_the_locked_brief; do_not_reinterpret_selection_or_component_keys",
            }
        else:
            result = {
                "schema_version": "pptx-studio-replay-report.v1",
        ...
        _write_json(args.replay_output, result)
        return {"status": result["status"], "replay_output": str(args.replay_output), "summary": {"code": result.get("code")}}
    if args.command == "adapt":
        ...
        result = compile_adaptation(
            _read_json(args.composition_plan), catalog=catalog,
            request=_read_json(args.adaptation_input),
            preflight=_read_json(args.preflight_output), component_profiles=component_profiles,
        )
    ...
    if args.command == "validate-narrative":
        if args.brief_normalized is None or args.narrative_input is None or args.narrative_output is None:
            raise ValueError("NARRATIVE_ARGUMENT_REQUIRED")
        result = validate_narrative_plan(
            _read_json(args.brief_normalized), _read_json(args.narrative_input),
        )
        _write_json(args.narrative_output, result)
        return {
            "status": result["status"],
            "narrative_output": str(args.narrative_output),
            "summary": {"slide_count": result["slide_count"], "brief_id": result["brief_id"]},
        }
    if args.command == "preflight":
        ...
        result = preflight_native_slots(
            _read_json(args.composition_plan), catalog=catalog,
            private_source_root=args.private_source_root, component_profiles=component_profiles,
        )
    ...
    if args.command == "vision-plan":
        ...
        plan = plan_visual_batches(
            ...
            batch_size=args.batch_size, page_ids=args.page_id,
        )
