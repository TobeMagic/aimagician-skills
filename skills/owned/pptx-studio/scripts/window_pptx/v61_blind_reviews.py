...
PROTOCOL_ID = "pptx-studio-v61-blind-v1"
...
def _segment_prompt(
    *,
    ...
    return (
        ...
        "The reference does not establish the correct candidate wording or data: "
        "those may intentionally differ. Do not infer a truncation or malformed "
        "word from a reference/candidate copy difference or low-resolution OCR. "
        "Report such a defect only when a clipped glyph, collision, or broken line "
        "is directly unambiguous in the candidate image.\n\n"
...
def _synthesis_prompt(
    *,
    ...
    return (
        ...
        "never cite a reference page as if it were a candidate defect. A "
        "Blocker or Important finding requires an explicit, material "
        "candidate-versus-reference regression on the same slide; an inherited "
        "reference characteristic is not a blocking defect. Candidate business "
        "copy may intentionally differ from reference copy, so wording differences "
        "and uncertain OCR are not evidence of a visual defect; only directly "
        "visible clipping, collision, or broken lines may support one. Set "
