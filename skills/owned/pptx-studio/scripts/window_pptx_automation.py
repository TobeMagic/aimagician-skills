#!/usr/bin/env python3
"""Conservative Windows PowerPoint COM helper for the pptx-studio skill."""
...
"""Project-specific entrypoint for pptx-studio automation.
...
pptx-studio skill script or copied project helpers.
"""
...
def die(message: str, code: int = 1) -> None:
    print(f"pptx-studio: {message}", file=sys.stderr)
...
def init_project_workspace(project_dir: Path) -> dict[str, Any]:
    ...
    runner_path = project_dir / ".pptx-studio" / "scripts" / "run_project.py"
...
def ascii_temp_copy_path(project_dir: Path, source: Path) -> Path:
    return project_dir / ".pptx-studio" / "temp" / f"deck_temp_ascii{source.suffix}"
...
def append_asset_manifest(project_dir: Path, rows: list[dict[str, Any]]) -> Path:
    manifest_path = project_dir / ".pptx-studio" / "asset_manifest.json"
...
def pixabay_search(args: argparse.Namespace, project_dir: Path) -> dict[str, Any]:
    ...
    request = Request(
        ...
        headers={"User-Agent": "pptx-studio-skill/1.0"},
    )
    ...
    cache_dir = project_dir / ".pptx-studio" / "cache" / "pixabay"
...
def download_image(project_dir: Path, url: str, source_row: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    request = Request(url, headers={"User-Agent": "pptx-studio-skill/1.0"})
...
def iconify_search(args: argparse.Namespace, project_dir: Path) -> dict[str, Any]:
    ...
    cache_dir = project_dir / ".pptx-studio" / "cache" / "iconify"
    cache_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
...
def audit_presentation(presentation: Any, project_dir: Path) -> dict[str, Any]:
    ...
    audit_dir = project_dir / ".pptx-studio" / "audits"
...
def add_request_summary_slide(presentation: Any, request_text: str, template: Path | None) -> None:
    ...
    footer.TextFrame.TextRange.Text = (
        "Generated by pptx-studio helper at "
...
def main(
    ...
    *,
    ...
    if args.render_assembly_plan:
        try:
            ...
            report_path = (
                ...
                else project_dir / ".pptx-studio" / "audits" / "physical-assembly-report.json"
            )
            ...
            qa_path = (
                ...
                else project_dir / ".pptx-studio" / "audits" / "physical-rule-qa.json"
            )
    ...
    if portable_render_requested:
        ...
        if args.export_slides:
            ...
            export_dir = (
                ...
                or (project_dir / ".pptx-studio" / "exports")
            )
            export_dir.mkdir(parents=True, exist_ok=True)
        ...
        if args.export_qa:
            qa_dir = project_dir / ".pptx-studio" / "exports" / "qa"
    ...
    if args.extract_media and args.no_output_deck and not args.export_slides:
        ...
        media_dir = resolve_path(project_dir, args.media_dir) or (project_dir / ".pptx-studio" / "media")
    ...
    if not com_needed:
        ...
        if not args.json:
            print("pptx-studio non-COM run complete")
    ...
    if args.render_brief_plan and prepared_brief_generation is not None:
        ...
        write_brief_generation_artifacts(
            prepared_brief_generation,
            project_dir / ".pptx-studio" / "audits",
        )
    ...
    try:
        ...
        if args.render_deck_plan or args.render_brief_plan:
            ...
            if args.export_qa:
                rendered_qa_result = export_all_slides_to_png(
                    presentation,
                    project_dir / ".pptx-studio" / "exports" / "qa",
                )
        ...
        if args.export_slides:
            export_dir = resolve_path(project_dir, args.export_dir) or (project_dir / ".pptx-studio" / "exports")
        ...
        if not args.json:
            print("pptx-studio run complete")
        emit_result(result, args.json, sys.stdout, sys.stderr)
    finally:
