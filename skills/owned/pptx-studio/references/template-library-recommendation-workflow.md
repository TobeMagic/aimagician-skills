...
Use this workflow when selecting reusable slide templates from the `pptx-studio` skill's built-in template library.
...
```powershell
py D:\Growth_up_youth\repo\skills\skills\owned\pptx-studio\scripts\window_pptx_automation.py --project-dir D:\Growth_up_youth\repo\skills\skills\owned\pptx-studio --intake-template-library --no-save --json
```
