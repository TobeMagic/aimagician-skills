/**
 * Narration-led React stage with Scene, Cue, subtitle and karaoke helpers.
 * Interactive playback follows the audio element. Deterministic capture uses
 * window.__VISUAL_READY__ and window.__setDesignTime(seconds).
 * Keep the persistent hero outside Scene wrappers so the story can morph
 * continuously instead of cutting between slide-like layouts.
 */
...
const NarrationStageLib = (() => {
  ...
  function NarrationStage({
    ...
    React.useEffect(() => {
      ...
      window.__setDesignTime = (seconds) => {
        setCaptureMode(true);
        setPlaying(false);
        setTime(Math.max(0, Math.min(Number(seconds) || 0, timeline.totalDuration)));
      };
      Promise.resolve(document.fonts?.ready).then(() => {
        window.__VISUAL_READY__ = true;
      });
      return () => {
        delete window.__setDesignTime;
   ...
   *
   * Required: timeline.scenes[].chunks[] from the project narration timeline.
   *
   * Props（可覆盖默认样式）：
