/**
 * React Stage/Sprite timeline with interactive playback and deterministic capture.
 * Exposes window.__VISUAL_READY__ and window.__setDesignTime(seconds).
 */
...
(function() {
  ...
  function Stage({ duration = 10, width = 1920, height = 1080, fps = 60, loop = true, children, bgColor = '#fff' }) {
    ...
    const effectiveLoop = captureMode ? false : loop;
    ...
    useEffect(() => {
      ...
      window.__setDesignTime = (seconds) => {
        setCaptureMode(true);
        setPlaying(false);
        setTime(Math.max(0, Math.min(Number(seconds) || 0, Math.max(0, duration - 0.001))));
      };
    ...
    }, [playing, duration, effectiveLoop, captureMode]);
    ...
    return (
      <TimeContext.Provider value={ctx}>
        <div style={stageStyles.wrapper}>
          ...
          {!captureMode && <div style={stageStyles.controls}>
            <button
