...
| **V61-SEL-01** | Candidate selection can rank unsafe pages and style filtering has no discriminating value | Apply direct-use, asset-presence, residue-risk, and style-compatibility eligibility gates, then deterministically rank survivors by role, capacity, semantics, style, and editability under one dominant compatible style family | Repeated serialized queries are byte-identical; every result exposes eligibility, gate reasons, five score components and total; components use 0.30/0.25/0.20/0.15/0.10 exactly; missing assets, residue risk, reference-only pages, and incompatible fallback fail closed |
...
| **V61-ADAPT-01** | Text replacement works on the chosen slide but is not protected by complete dependency closure, direct-use eligibility, or source-bound fact evidence | Adapt only declared native text slots after dependency closure, require every replacement to bind allowed locked fact/asset IDs, enforce slot capacity before mutation, and preserve geometry, theme, editable charts/media, and immutable facts | Evidence records source shape/text hash, replacement hash, fact/asset refs, capacity used/limit; invented or missing facts, unbound literals, unknown slots, residue, over-capacity text, or mutation outside declared shape IDs fail; accepted output opens through `python-pptx` and LibreOffice |
...
| **V61-REL-01** | v6.1 has no accepted clean-room Codex run or fresh closure audit | Close only after engineering gates, three independent visual reviews, a frozen-point premerge implementation audit, merge/push, installed Skill parity, and a second completion audit frozen to pushed master | Every Phase 49 goal is PASS, visual reviews meet the threshold with no Blocker/Important, the premerge audit approves, master is merged/pushed, source/install digests match, and the final audit of that exact pushed SHA returns DONE with zero Blocker/Important |
